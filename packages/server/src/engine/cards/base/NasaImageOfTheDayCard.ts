...
/**
 * Card No.92 — NASA Image of the Day.
 * Gain 2 publicity, plus 1 publicity for each tucked card-income card,
 * then tuck this card for income.
 */
export class NasaImageOfTheDay extends ImmediateCard {
  public constructor() {
    super(loadCardData('92'), {
      behavior: { gainResources: { publicity: 2 } },
    });
  }
    ...
    context.game.deferredActions.push(
      new SimpleDeferredAction(
        ...
        () => {
          ...
          if (gained > 0) {
            context.player.resources.gain({ publicity: gained });
          }
