...
import { ParkesObservatoryCard } from '@/engine/cards/base/ParkesObservatoryCard.js';
...
function createGame(_player: Player): {
  ...
  const sectors = [createSector(ESector.RED, 0), createSector(ESector.BLUE, 1)];
  const game = {
    sectors,
...
describe('ParkesObservatoryCard (card 52)', () => {
  it('loads expected metadata without exposing desc custom token', () => {
    const card = new ParkesObservatoryCard();

    expect(card.id).toBe('52');
    expect(card.behavior.custom).toBeUndefined();
  });

  it('performs scan and scores 2 VP for each red signal marked by the scan', () => {
    const card = new ParkesObservatoryCard();
  ...
  it('scores red signals from scan tech sub-actions in the same scan flow', () => {
    const card = new ParkesObservatoryCard();
    const player = new Player({
      id: 'p1',
      name: 'Alice',
    ...
    const done = afterCardRow?.process({
      type: EPlayerInputType.OPTION,
