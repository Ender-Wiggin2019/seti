...
import { VeritasTelescopesCard } from '@/engine/cards/base/VeritasTelescopesCard.js';
...
describe('VeritasTelescopesCard (card 54)', () => {
  it('loads expected metadata without exposing desc custom token', () => {
    const card = new VeritasTelescopesCard();

    expect(card.id).toBe('54');
    expect(card.behavior.custom).toBeUndefined();
  });

  it('performs scan and scores 2 VP for each blue signal marked by the scan', () => {
    const card = new VeritasTelescopesCard();
