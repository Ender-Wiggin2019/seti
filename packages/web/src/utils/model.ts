import { ICorp, IModel } from '@seti/common/types/corp';
import { EEffectType, Effect, IBaseEffect } from '@seti/common/types/effect';
import {
  EMiscIcon,
  EResource,
  EScanAction,
...
export const getModel = (corp: ICorp, round: number): IModel => {
  ...
  const freeActionValue =
    freeActionUnitValue !== undefined
      ? freeActionUnitValue * round
      : getValuesFromEffects(corp.freeAction || []);
