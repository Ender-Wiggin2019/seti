...
function randomizeWheels() {
  ...
  for (let w of [1, 2, 3, 4]) {
    let oldRotation = document.querySelector(".wheel" + w).getAttribute("currRotation")
    let newRotation = +oldRotation - Math.floor(Math.random() * 8 + [0, 20, 11, 7, 4][w])
  ...
  document.querySelector(".connector").animate([
    {filter: "brightness(1)"},
    {filter: "brightness(3)"},
    {filter: "brightness(1)"},
  ], {duration: 1000})
...
function randomizeSectors() {
  for (let w of [1, 2, 3]) {
    ...
    while (randomSectors.length) {
      let targetWrap = document.getElementById("sector-wrap-" + randomSectors.length)
      targetWrap.innerHTML = ""
      let randomSector = randomSectors.splice(Math.floor(Math.random() * randomSectors.length), 1)[0]
...
function main() {
  ...
  initWheels()
  //randomizeSectors()
  document.querySelector(".spin-button").addEventListener("click", randomizeWheels)
}
