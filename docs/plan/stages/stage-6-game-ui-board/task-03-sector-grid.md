...
## Title
实现扇区环形视图 (8 扇区 + 数据槽 + 信号标记 + 完成状态)

## 描述
实现客户端的扇区棋盘可视化：8 个扇区围绕太阳系最外圈布局（参考 frontend-reference 的 sky 位置），每个扇区内显示数据槽位、信号标记、溢出标记、胜出标记和完成状态。支持 SelectSector PlayerInput 的交互高亮。
...
## 完成标准
- [x] 8 扇区正确渲染（使用信号标记静态资源）
- [x] 数据和标记状态准确反映
- [x] 选择交互高亮工作（SelectSector 仅高亮可选项）
- [x] 扇区进度和竞争态势展示（数据槽/标记可视化）
- [x] 所有单测通过
